...
const Doughnuts = ()=>{
    ...
    const [dataD,setDataD] = useState<{current:number,total:number,month:string}[]>([])
    useEffect(()=>{
        setDataD(
            [
                {
                    ...
                    dataD.map((item,i)=>{
                        return (
                            <div className='max-w-[33%] flex flex-wrap' key={i}>
                                <h1 className='w-full text-2xl flex justify-center items-center'>{item.month}</h1>
                                <Doughnut data={getData(item.current, item.total)} className=''/>
                            </div>
                            )
                    })
                :
